"""
This code uses the implicit Domain-Agnostic Fourier Neural Operator (iDAFNO) on the airfoil dataset described
...
if __name__ == '__main__':
    ...
    mask = np.load('./data_airfoil_naca/NACA_mask_interp.npy')
    mask = torch.tensor(mask, dtype=torch.float)
    dist_in = np.load('./data_airfoil_naca/NACA_dist.npy')
    dist_in = torch.tensor(dist_in[:Ntotal, :, :], dtype=torch.float)
    input = smooth_chi(mask, dist_in, smooth_coef)
    # input = mask
    output = np.load('./data_airfoil_naca/NACA_Q_interp.npy')
            ...
            for wd in wds:
                for isd in seeds:
                    ...
                    for i in range(layer_end):
                        ...
                        if nb != 1:
                            restart_nb = nb // 2
                            model_filename_res = '%s/model_depth%d.ckpt' % (base_dir, restart_nb)
                            if torch.cuda.is_available():
                        ...
                        model_filename = '%s/model_depth%d.ckpt' % (base_dir, nb)
                            ...
                            if train_l2 < best_train_loss:
                                ...
                                if test_l2 < best_test_loss:
                                    ...
                                    print(f'>> s: {smooth_coef}, depth: {nb}, '
                                          ...
                                          f'runtime: {(t2 - t1):.2f}s, train loss: {train_l2:.5f}, '
                                          f'validation loss: {test_l2:.5f}')
                                else:
                                    early_stop += 1
                                    t2 = default_timer()
                                    print(f'>> s: {smooth_coef}, depth: {nb}, '
